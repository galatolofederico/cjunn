...
class MLP4Model(torch.nn.Module):
    def __init__(self, inputs, hidden, outputs, activation=torch.nn.functional.leaky_relu, seed=None, pruning=None):
        ...
        self.hidden3_to_hidden4 = torch.nn.Linear(self.hidden, self.hidden)
        self.hidden4_to_out = torch.nn.Linear(self.hidden, self.outputs)
    ...
    def forward(self, X):
        ...
        h4 = self.activation(self.hidden3_to_hidden4(h3))
        y = self.activation(self.hidden4_to_out(h4))
    ...
    def compute_prune_masks(self):
        ...
        self.hidden3_to_hidden4_weight_mask = self.compute_prune_mask(self.hidden3_to_hidden4.weight)
        self.hidden3_to_hidden4_bias_mask = self.compute_prune_mask(self.hidden3_to_hidden4.bias)

        self.hidden4_to_out_weight_mask = self.compute_prune_mask(self.hidden4_to_out.weight)
        self.hidden4_to_out_bias_mask = self.compute_prune_mask(self.hidden4_to_out.bias)
    ...
    def prune(self, compute_masks=True):
        ...
        self.hidden3_to_hidden4.weight.data = self.hidden3_to_hidden4.weight.data*self.hidden3_to_hidden4_weight_mask
        self.hidden3_to_hidden4.bias.data = self.hidden3_to_hidden4.bias.data*self.hidden3_to_hidden4_bias_mask

        self.hidden4_to_out.weight.data = self.hidden4_to_out.weight.data*self.hidden4_to_out_weight_mask
        self.hidden4_to_out.bias.data = self.hidden4_to_out.bias.data*self.hidden4_to_out_bias_mask
    ...
    def backward(self, loss, optimizer, optimizer_idx, *args, **kwargs):
        ...
        if self.pruned:
            ...
            self.hidden3_to_hidden4.weight.grad = self.hidden3_to_hidden4.weight.grad*self.hidden3_to_hidden4_weight_mask
            self.hidden3_to_hidden4.bias.grad = self.hidden3_to_hidden4.bias.grad*self.hidden3_to_hidden4_bias_mask

            self.hidden4_to_out.weight.grad = self.hidden4_to_out.weight.grad*self.hidden4_to_out_weight_mask
            self.hidden4_to_out.bias.grad = self.hidden4_to_out.bias.grad*self.hidden4_to_out_bias_mask